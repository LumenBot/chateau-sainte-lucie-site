---
order: 2
nav: Feuillage
name: Suite Feuillage
seo:
  title: Suite Feuillage | Les Nuits au Château
  description: >-
    La plus enveloppante des deux suites de Sainte-Lucie : verts profonds, bois
    sombre et laiton, une suite profonde et une grande terrasse à balustrade ouverte
    sur les frondaisons. Ouverture avril 2027.
hero:
  eyebrow: Suite Feuillage
  title: Feuillage, la profondeur des frondaisons.
  lead: >-
    La plus enveloppante. Verts profonds, bois sombre et laiton ; la pénombre y
    devient une qualité — lecture, bain chaud, lumière basse et vue sur les arbres.
  image: projFeuillageChambre
  imageAlt: Suite Feuillage — chambre principale (visuel de projection)
  imagePosition: "center 55%"
palette: Verts profonds, bois sombre et laiton.
ambiance:
  - Verts profonds
  - Bois sombre
  - Laiton
  - Pénombre choisie
intro: >-
  La plus enveloppante. Verts profonds, bois sombre et laiton composent une
  suite ouverte sur une grande terrasse à balustrade. Feuillage assume la
  pénombre et la transforme en qualité : lecture, bain chaud, lumière basse et vue
  sur les arbres.
composition:
  - "Une chambre principale et une configuration préparée sur demande pour un ou deux enfants"
  - Une salle de bain minérale
  - Une grande terrasse privative à balustrade
aConfirmer: >-
  Les équipements détaillés et les modalités d'accès seront communiqués à
  l'ouverture des réservations. Tarif : 200 € la nuit pour deux adultes,
  petit-déjeuner inclus ; jusqu'à deux enfants peuvent être accueillis.
tagline: La plus enveloppante — verts profonds, bois sombre, ouverte sur les frondaisons.
cardImage: projFeuillageChambre
cardPosition: "center 50%"
renders:
  - { image: projFeuillageBain, caption: "La salle de bain minérale" }
  - { image: projFeuillageTerrasse, caption: "La grande terrasse à balustrade" }
---
